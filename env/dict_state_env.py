...
'''
Environment with full state (dictionary)
Arguments:
    path: path to the dictionary.npz and decoders.npz files
    reward_func: function that takes in a vector of activations and returns a scalar reward
    score_func: function that takes in a state and returns a score
    n_maxstep: maximum number of steps per episode
...
class DictionaryStateEnvironment:
    def __init__(self, path, reward_func, score_func, n_maxstep, n_elecs, n_amps):
        ...
        try:
            ...
            raise Exception("Please make sure the dictionary.npz and decoders.npz files are present in the specified path")
